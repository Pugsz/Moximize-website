              ...
              <div className="mt-4">
                ...
                {billingCycle === 'annually' && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Billed annually (${plan.priceAnnually.toLocaleString()})
                  </p>
                )}
                {billingCycle === 'annually' && (
                    <p className="text-xs text-muted-foreground line-through opacity-70 mt-1">
                        ${originalMonthlyPrice.toLocaleString()}/mo
                    </p>
                )}
              </div>
            </CardHeader>
...
// ── Moximize pricing data ────────────────────────────────────────────────────
const moximizePlans: [PriceTier, PriceTier, PriceTier] = [
  {
    id: 'basic',
    name: 'Basic',
    description: 'For early-stage companies building their first growth system.',
    priceMonthly: 1200,
    priceAnnually: 12000,
    isPopular: false,
    buttonLabel: 'Get Started',
    features: [
      { name: 'HubSpot CRM setup & configuration', isIncluded: true },
      { name: '1 active marketing campaign', isIncluded: true },
      { name: 'Monthly performance reporting', isIncluded: true },
      { name: 'Email support (48h response)', isIncluded: true },
      { name: 'Up to 500 contacts managed', isIncluded: true },
      { name: 'Dedicated account manager', isIncluded: false },
      { name: 'Bi-weekly strategy calls', isIncluded: false },
      { name: 'LinkedIn + email outbound', isIncluded: false },
      { name: 'Custom HubSpot integrations', isIncluded: false },
      { name: 'Priority support (4h response)', isIncluded: false },
    ],
  },
  {
    id: 'growth',
    name: 'Growth',
    description: 'For scaling companies ready to build a full revenue engine.',
    priceMonthly: 2000,
    priceAnnually: 21600,
    isPopular: true,
    ...
    features: [
      ...
      { name: 'Bi-weekly strategy calls', isIncluded: true },
      { name: 'LinkedIn + email outbound', isIncluded: true },
      { name: 'Custom HubSpot integrations', isIncluded: false },
      { name: 'Priority support (4h response)', isIncluded: false },
    ],
  },
  {
    id: 'premium',
    name: 'Premium',
    description: 'For established teams that need comprehensive, hands-on support.',
    priceMonthly: 2800,
    priceAnnually: 30000,
    isPopular: false,
    ...
    features: [
      ...
      { name: 'Bi-weekly strategy calls', isIncluded: true },
      { name: 'LinkedIn + email outbound', isIncluded: true },
      { name: 'Custom HubSpot integrations', isIncluded: true },
      { name: 'Priority support (4h response)', isIncluded: true },
    ],
  },
];

const MoximizePricingDemo = () => {
...
export { PricingComponent, moximizePlans, type BillingCycle, type PriceTier };
export default MoximizePricingDemo;
